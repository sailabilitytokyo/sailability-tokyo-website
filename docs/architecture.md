...
- 翻訳ファイル（`src/content/pages/<言語>/<ページ>.md`）を置いたページだけが生成される。日本語以外のメニュー・フッターには、その言語の翻訳があるページだけを出す（日本語ページに飛ばすと「リンク切れ」と感じるため。2026-09-24）。
- 最初は観光客向けの **トップ・セーリング体験会・About** だけを翻訳した（2026-09-23）。メニューから日本語ページに移動するのが分かりにくかったため、**小学生ヨット教室・会員募集・プライバシーポリシーも翻訳**した（2026-09-24）。About は要点をまとめた版、プライバシーポリシーは参考訳（日本語版が優先）。
- お知らせは日本語のみ（毎回の翻訳は負担が大きいため）。翻訳ページではメニューに出さず、フッターに「News (in Japanese)」として日本語の一覧へのリンクを置く。
- 言語の切り替えはヘッダーの上の帯。そのページの翻訳がなければ、その言語のトップページへ移動する。
- 中国語のページは、字形に合わせて Noto Sans SC（簡体字）/ TC（繁体字）を読み込む。
- 埋め込みの Googleマップも言語に合わせて表示する（`hl=en` など）。
- 予約フォーム（Google フォーム）は日本語のみ。翻訳ページでは当日参加もできることを案内している。
...
- **Cookie の同意バナーは付けない**（2026-09-24 決定）。日本の法令ではアクセス解析のみなら不要で、EU の GDPR が厳密に適用される可能性は低いため。EU が必須にするなど状況が変わったら見直す。代わりに、プライバシーポリシーに Google Analytics・Cookie・オプトアウト方法・外部サービス（Googleマップ・Google Fonts）を明記した（GA の利用規約でも必要）。Google シグナルは OFF にする。
...
- 文章をサイト全体で見直し、句読点・表記を統一した（ルールは AGENTS.md「文章のルール」）。About の誤字も修正
- セーリング体験会: 4歳未満は無料（子ども用ライフジャケットを着用できれば乗船可）と確認。予約フォームへのリンクを常に表示
- 小学生ヨット教室: 「江東区 後援」をページ上部の札で表示
- 会員募集: 法人会員は社員の方も個人会員と同様に乗船できること、会費を2年以上滞納した場合の扱いを追記
- セーリング体験会: デザイン見本をもとに「当日の流れ」「よくあるご質問」を追加。ライフジャケット（股ひも含む）を着けられない方はお断り、子ども用は2歳くらいから用意、スカートは非推奨。見本の未確認の事実（前日18時の中止連絡・予約割引・説明5分・乗り移りの手伝い）は載せていない
- トップの見出し「海に出よう。誰でも、今日から。」はそのまま使うことで確認済み
- 仮公開サイトのメンバー確認は完了（2026-09-23）。車いすの乗り移りの写真はない
- 体験会・ヨット教室に Googleマップの埋め込み（APIキー不要の埋め込み URL）を追加
- 多言語化（英語・簡体字・繁体字）を実施。メニューの「Membership」は日本語では「会員募集」に
- 日本語のメニューの「About」は「セイラビリティ東京について」に（「団体について」は分かりにくいため。2026-09-24）。メニューが長くなったので、日本語ページのロゴ横の小さな団体名表記は出さず、メニューを折りたたむ幅を 64rem → 70rem に広げた
- 翻訳のないページにメニューから飛ぶと日本語ページが開き「リンク切れ」に見えたため、小学生ヨット教室・会員募集・プライバシーポリシーも翻訳した（2026-09-24）
- ファビコンは「Sailability Tokyo」の文字入りの縦組みロゴから作成（鳥のマーク単体は使わない決まりのため。`scripts/make-favicons.mjs`）
- ハンザは「ひっくり返らない（設計の）ヨット」と書いてよいと確認済み。ライフジャケットは体に合ったサイズと股ひもが必須（落水時に脱げたり頭が沈んだりするのを防ぐため）。中止の連絡方法は書かない
